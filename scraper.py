"""
LUNVIA 標案情報自動爬蟲 v3 (requests 版)
用 HTTP POST 直接搜尋，不需要 Playwright 瀏覽器
"""
...
def search_keyword(keyword):
    ...
    try:
        ...
        for row in rows:
            ...
            if not any(d in date_text for d in RECENT_DATES):
                continue
...
def main():
    ...
    print(f"LUNVIA 標案爬蟲啟動 v3 (requests) — 抓最近3天 ({dates_str})")
    tenders = scrape_all()
    if not tenders:
